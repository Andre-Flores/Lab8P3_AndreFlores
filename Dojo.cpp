...
void Dojo::combate(int indice1, int indice2) {
	double porcentajeCinta= 0.0;
	double porcentajeCinta2= 0.0;
	if (indice1 >= 0 && indice1 < estudiantes.size() &&
		...
		while (student1->getSalud() > 0 && student2->getSalud() > 0) {
			...
			double ataque1 = student1->getFuerza() * porcentajeCinta * (student1->getSalud()*0.1 / 10.0);
			cout << "Ataque del Estudiante 1 = " << ataque1 << endl;
			double defensa2 = student2->getResistencia() * porcentajeCinta2 * (student2->getSalud()*0.1/ 10.0);
			...
			cout << "vida del estudiante 2 después de recibir el ataque = " << student2->getSalud() << endl;
			if (student2->getSalud() <= 0) {
				cout << student2->getNombre() << " ha sido derrotado!" << endl;
				break;
			}
			...
			double ataque2 = student2->getFuerza() * porcentajeCinta2 * (student2->getSalud()*0.1 / 10.0);
			cout << "Ataque del Estudiante 2 = " << ataque2 << endl;
			double defensa1 = student1->getResistencia() * porcentajeCinta * (student1->getSalud()* 0.1 / 10.0);
			...
			cout << "vida del estudiante 1 después de recibir el ataque = " << student1->getSalud() << endl;
			if (student1->getSalud() <= 0) {
				cout << student1->getNombre() << " ha muerto" << endl;
				break;
			}
		}
	}
